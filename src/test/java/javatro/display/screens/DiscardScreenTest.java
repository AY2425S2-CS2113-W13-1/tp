...
public class DiscardScreenTest {
    ...
    public void testCommandMap() {
        ...
        assertEquals(
                5,
                discardScreen.getCommandMap().size(),
                "DiscardScreen should have exactly 5 options in the command map");

        List<String> commandDescriptions =
                discardScreen.getCommandMap().stream().map(Option::getDescription).toList();

        // Check for all expected commands
        assertTrue(
                commandDescriptions.contains("Select Cards"),
                "Command map should contain 'Select Cards' option");
        assertTrue(
                commandDescriptions.contains("Sort cards by Suit"),
                "Command map should contain 'Sort by Suit' option");
        assertTrue(
                commandDescriptions.contains("Sort cards by Rank"),
                "Command map should contain 'Sort by Rank' option");
        assertTrue(
                commandDescriptions.contains("View Poker Hands"),
                "Command map should contain 'View Poker Hands' option");
        assertTrue(
                commandDescriptions.contains("Return To Game"),
                "Command map should contain 'Resume Game' option");
    }
